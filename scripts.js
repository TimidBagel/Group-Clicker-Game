...
const mainPlayer = new Player(); // instantiates player
...
const mainDog = new Dog(); // instantiates dog

// depletes player's food, increments dog's fullness
function FeedDog(){
    if(mainPlayer.food >= mainDog.feedAmount){
        mainPlayer.food -= mainDog.feedAmount
        mainDog.fullness += 1
        mainDog.feedAmount += ((mainDog.feedAmount/100)*mainDog.foodFlation) // inflates food required by dog
...
function AddPlayerEffects({ inflation = 0, food = 0, foodCap = 0, modifiers = [], Buildings = [] , stability=0, buildingsLost = 0, feedAmount = 0} = {}) {

    mainDog.feedAmount += feedAmount
...
// purchases a building, adds it to list of player-owned buildings
...
// purchases an upgrade, adds the apropriate modifier to the player
